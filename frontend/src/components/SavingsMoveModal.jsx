...
export default function SavingsMoveModal({ account, mode, onClose, onConfirm }) {
  ...
  async function handleConfirm() {
    ...
    if (!isDeposit && Number(amount) > account.balance) return setError("Số dư không đủ để rút.");
    setError("");
    setBusy(true);
    try {
      ...
      <div style={{ background: theme.color.surface, borderRadius: theme.radius.lg, padding: 22, width: 340, boxSizing: "border-box", boxShadow: theme.shadow.pop }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
          <span style={{ fontWeight: 700, fontSize: 16, color: theme.color.text }}>
            {isDeposit ? "Nạp tiền vào" : "Rút tiền từ"} {account.name}
          </span>
          <button onClick={onClose} style={{ background: "none", border: "none", cursor: "pointer", color: theme.color.textMuted }}><X size={18} /></button>
        </div>
        <div style={{ fontSize: 12, color: theme.color.textMuted, marginBottom: 16 }}>Số dư hien tai: {fmt(account.balance)}</div>
        ...
        <input value={note} onChange={(e) => setNote(e.target.value)} style={inputStyle} placeholder="Vi du: Trich tu luong thang 8" />
        ...
        <button onClick={handleConfirm} disabled={busy} style={{
          ...
          background: isDeposit ? theme.color.primary : theme.color.text, color: "#fff",
          fontSize: 13.5, fontWeight: 600, cursor: "pointer", marginTop: 4,
        }}>
          {busy ? "Đang xử lý..." : isDeposit ? "Xác nhận nạp tiền" : "Xác nhận rút tiền"}
        </button>
      </div>
    </div>
  );
